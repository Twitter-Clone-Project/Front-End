...
describe('RetweetersList', () => {
  it('renders the component successfully', () => {
    ...
    expect(getByTestId(`retweeters-list`)).to.exist;
    expect(getByTestId(`retweeters-list-backbtn`)).to.exist;
    expect(getByTestId(`retweeters-list-1`)).to.exist;
    expect(getByTestId(`retweeters-list-2`)).to.exist;
    // expect(getByTestId(`${reply[0].replyId}menu`)).not.to.exist;
  });

  it('gets the retweeters successfully', async () => {
    render(
      <AuthProvider value={{ dispatch, user: null, isAuthenticated: true }}>
        <BrowserRouter>
          <ProtectedRoute>
    ...
    await waitFor(() => {
      ...
      expect(window.fetch).toHaveBeenCalledWith(
        `${import.meta.env.VITE_API_DOMAIN}tweets/undefined/retweeters`,
        {
          method: 'GET',
          origin: true,
          credentials: 'include',
  ...
  it('navigates back on click on the back button', () => {
    ...
    const backButton = getByTestId(`retweeters-list-backbtn`);
